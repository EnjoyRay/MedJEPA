...
![Full input perturbation grid](results/report_visualizations/fig_input_perturbation_grid.png)

> 这张网格补充展示 Exp1 的完整输入扰动族：高斯噪声、高斯模糊、亮度偏移和对比度缩放。它解释了为什么本报告把“几乎不可见的轻噪声”和“肉眼可见的强扰动”分开讨论。
...
| **MoCo v3 ViT-B/16** | ViT-Base patch16, 768-dim | MIMIC-CXR, 300 epochs | 0.684 |

> **注意**：MoCo 使用 ViT-Base（768-dim, 86M 参数），而其他模型使用 ViT-Huge（1280-dim, 632M 参数）。架构差异意味着绝对性能不可直接比较，但噪声鲁棒性和病灶敏感性的**行为模式**反映的是自监督方法的特性，可跨架构对比。
...
![Exp1 full perturbation heatmap](results/report_visualizations/fig_exp1_full_perturbation_heatmap.png)

> Exp1 全扰动热图同时展示 AUROC、AUROC drop 和 cosine drift。可以看到 JEPA300 的主要异常集中在高斯噪声的表征漂移；模糊、亮度和对比度扰动也会造成下降，但形态更连续。
...
![Exp3 per-class frequency heatmap](results/report_visualizations/fig_exp3_per_class_frequency_heatmap.png)

> Per-class 频率热图显示，中频带损伤不是单一类别造成的偶然现象，而是多数类别上 JEPA300 都低于 MAE300；这支持“频率机制是模型表征层面的系统性问题”这一解释。
...
![Exp6 NCA training details](results/report_visualizations/fig_exp6_nca_training_details.png)

> NCA 细节图补充展示训练 loss、JEPA300 的一致性损失分量，以及 raw encoder drift 到 adapter output drift 的压缩幅度。
...
![Exp2b per-sample lesion-control distribution](results/report_visualizations/fig_exp2b_per_sample_distribution.png)

> Per-sample 分布图说明 JEPA300 的病灶敏感性并不只是均值被少数极端样本拉高；在多个高响应类别上，lesion-control delta 的整体分布都比 MAE300 更偏正。
...
![Exp4 per-sample mechanism scatter](results/report_visualizations/fig_exp4_per_sample_mechanism_scatter.png)

> Exp4 per-sample scatter 把 token 面积、显著性比例、噪声漂移和病灶漂移放在同一视角下。JEPA300 的噪声漂移高但 lesion drift 低，MAE300 则更接近连续漂移形态。
...
5. **MoCo 架构不匹配**：MoCo 使用 ViT-Base（768-dim），其余模型为 ViT-Huge（1280-dim）。绝对性能差异有架构因素。

### 5.5 MoCo 基准：三种自监督范式的行为对比

MoCo v3 ViT-B/16（300 epochs, MIMIC-CXR）提供了第三类自监督方法的参照：

| 方法 | 训练目标 | Clean AUROC | σ=0.05 Drift | 病灶 Delta | 特征分布 |
|------|---------|:-----------:|:------------:|:----------:|---------|
| **I-JEPA** | 潜在表示预测 | 0.916 | 0.872 | **+0.195** | 局部集中 |
| **MAE** | 像素重建 | 0.891 | 0.169 | +0.039 | 全局均匀 |
| **MoCo** | 对比学习 | 0.684 | **0.014** | **+0.001** | 全局均匀 |

**行为模式**：

1. **噪声鲁棒性：MoCo ≈ MAE >> I-JEPA**。对比学习和像素重建都天然产生平滑的表征空间（MoCo 的 InfoNCE loss 和 MAE 的 MSE loss 都是连续函数）。I-JEPA 的潜在预测目标缺乏这种平滑性约束。

2. **病灶敏感性：I-JEPA >> MAE ≈ MoCo**。仅 I-JEPA 表现出有意义的病灶区域依赖。对比学习（MoCo）和像素重建（MAE）都倾向于使用全局特征进行分类，不特别关注病灶区域。

3. **关键推论**：病灶敏感性是 I-JEPA **特有的涌现属性**，不是自监督学习的通用特性。它源于 I-JEPA 的 context→target 预测机制，该机制迫使编码器关注信息密度最高的图像区域——这些区域恰好与放射科医生的病灶标注重叠。
...
| **v6 (+vicreg)** | 0.916 | **0.906** | 0.889 | **0.802** | 0.529 |
| MAE-H/300 | 0.891 | 0.777 | 0.618 | 0.490 | 0.485 |
| **MoCo v3 ViT-B/16** | 0.684 | 0.657 | 0.562 | 0.540 | 0.541 |

> MoCo 使用 ViT-Base（768-dim），其余模型为 ViT-Huge（1280-dim）。绝对值不可直接比较，关注噪声响应模式。
...
| v6 (+vicreg) | 0.010 | 0.017 | **0.064** | 0.212 |
| MAE-H/300 | 0.169 | 0.319 | 0.339 | 0.337 |
| **MoCo v3 ViT-B/16** | **0.014** | 0.053 | 0.083 | 0.094 |

> MoCo 的 drift 是所有模型中最小的（σ=0.05 时仅 0.014），甚至优于 MAE-H/300（0.169）。对比学习范式的天然平滑性在 ViT-Base 上已充分体现。
...
| MAE-H/97 | +0.066 | [0.053, 0.078] | *** |
| MAE-H/300 | +0.039 | [0.025, 0.053] | *** |
| **MoCo v3 ViT-B/16** | **+0.001** | [0.001, 0.002] | *** |

> MoCo 的 delta 接近零——病灶和对照遮挡几乎无法区分。MoCo 和 MAE 在"不使用病灶区域进行分类"这一点上高度一致。
...
| **MoCo v3 ViT-B/16** | −0.027 | −0.113 | −0.097 | MLP 提升 clean 但不改善噪声比例 |
